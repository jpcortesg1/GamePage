<div>
  <div class="comment ps-2 my-5" style="border-left: 2px solid #fff">
    ...
    <div class="bottom ms-3">
      ...
      <button type="button" class="btn btn-outline-light mt-2" data-bs-toggle="modal" data-bs-target="#exampleModal{{ $comment->getId() }}">
        Response
        <i class="fa-solid fa-reply"></i>
      </button>

      <!-- Modal -->
      <div class="modal fade" id="exampleModal{{ $comment->getId() }}" tabindex="-1" aria-labelledby="exampleModalLabel{{ $comment->getId() }}" aria-hidden="true">
              ...
              <form class="form" action="{{ route('comment.store', $comment->getIdGame()) }}" method="POST">
                ...
                <div class="form-floating text-light">
                  ...
                  <div class="input-group mt-3 visually-hidden">
                    <input type="text" name="idComment" class="form-control bg-dark text-light" placeholder="Recipient's username" aria-label="Recipient's username" aria-describedby="basic-addon2" value="{{ $comment->getId() }}">
                  </div>
