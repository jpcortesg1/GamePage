...
<form
  action="{{ route('admin.gameUpdate', $viewData['game']->getId()) }}"
  method="POST"
...
>
  ...
  <div class="mb-3">
    ...
    <select class="form-control" name="idCategory">
      ...
      <option value="{{ $category->getId() }}" {{ ($category->
        getId() == $viewData['game']->getIdCategory()) ? 'selected' : '' }} >{{
        $category->getName() }}
      </option>
      @endforeach
    </select>
